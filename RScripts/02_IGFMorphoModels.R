...
temp_w <- temp %>% select(-ForeLeg, -HornLen, -Horn, -Survival, -BredAsAYearling, -BirthWt, -Age) %>% drop_na()
temp_f <- temp %>% select(-Weight, -HornLen, -Horn, -Survival, -BredAsAYearling, -BirthWt, -Age) %>% drop_na()
temp_fw <- temp %>% select(-HornLen, -Survival, -Horn, -BredAsAYearling, -BirthWt, -Age) %>% drop_na()
temp_h <- temp %>% filter(Horn == 3) %>% select(-Weight, -ForeLeg, -Survival, -BredAsAYearling, -BirthWt, -Age) %>% drop_na()
temp_g <- temp %>% select(-ForeLeg, -Horn,  -HornLen, -Survival, -BredAsAYearling) %>% drop_na()
temp_hw <- temp %>% filter(Horn == 3) %>% select(-ForeLeg, -Survival, -BredAsAYearling, -BirthWt, -Age) %>% drop_na()
