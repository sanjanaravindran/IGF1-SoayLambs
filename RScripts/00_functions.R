...
prepare_data_list <- function(data) {
  ...
  if ("Age_sc" %in% names(data)) {
    data_list$Weight = data$Weight
    data_list$BirthWt_sc = data$BirthWt_sc
    data_list$Age_sc = data$Age_sc
  }
  return(data_list)
}
...
# Model processing to plot y~x and 500 draws from posterior (gaussian traits)
...
process_fit_growth_model_ma <- function(fit_mod, temp_data, variables, sample_variable) {
  ...
  mu.link <- function(IGF1_true)
    ...
    post_mod$beta_PopSize * mean(temp_data$PopSize_sc) +
    post_mod$beta_IGF * IGF1_true +
    post_mod$beta_Age * mean(temp_data$Age_sc) +
...
# Model processing to plot y~x and 500 draws from posterior (binomial model with weight as predictor too)
...
# Function to process posterior dist and get IGF_true values
...
# Violin plotting function comparing observed and corrected igf1 values
