...
# Rename DaysSinceBirth as Age
temp$Age <- temp$DaysSinceBirth

# Subset relevant columns
temp <- temp %>%
  dplyr::select(ID, Weight, ForeLeg, HornLen, Horn, Survival, BredAsAYearling, IGF1, SexF, Twin, PopSize, BirthYear, MumID, MumAge, ELISARunDate, PlateNumber, Age, BirthWt) 

# Split data into different subsets (weight, foreleg, growth)
temp_w <- temp %>% dplyr::select(-ForeLeg, -HornLen, -Horn, -Survival, -BredAsAYearling, -BirthWt, -Age) %>% drop_na()
temp_f <- temp %>% dplyr::select(-Weight, -HornLen, -Horn, -Survival, -BredAsAYearling, -BirthWt, -Age) %>% drop_na()
temp_fw <- temp %>% dplyr::select(-HornLen, -Survival, -Horn, -BredAsAYearling, -BirthWt, -Age) %>% drop_na()
temp_h <- temp %>% filter(Horn == 3) %>% dplyr::select(-Weight, -ForeLeg, -Survival, -BredAsAYearling, -BirthWt, -Age) %>% drop_na()
temp_g <- temp %>% dplyr::select(-ForeLeg, -Horn,  -HornLen, -Survival, -BredAsAYearling) %>% drop_na()
temp_hw <- temp %>% filter(Horn == 3) %>% dplyr::select(-ForeLeg, -Survival, -BredAsAYearling, -BirthWt, -Age) %>% drop_na()
...
temp_g <- standardize_columns(temp_g, c("Weight", "IGF1", "PopSize", "Age", "BirthWt", "MumAge"))
...
(p2 <- generate_and_plot_ppc(fit_mod_weight, "Weight_rep", temp_w, temp_w$Weight, "August Weight"))
(p1 <- generate_and_plot_ppc(fit_mod_foreleg, "ForeLeg_rep", temp_f, temp_f$ForeLeg, "August Foreleg Length"))
(p5 <- generate_and_plot_ppc(fit_mod_hornlen, "HornLen_rep", temp_h, temp_h$HornLen, "August Horn Length"))
(p4 <- generate_and_plot_ppc(fit_mod_growth, "Weight_rep", temp_g, temp_g$Weight, "August Weight \n(Also including lamb age and birth weight as predictors)"))
...
predictors_growth <- c("alpha", "beta_IGF", "beta_SexF", "beta_Twin", "beta_PopSize", "beta_Age", "beta_BirthWt", "beta_MumAge1", "beta_MumAge2", "sigma_e", "sigma_e1", "sigma_t", "sigma_u", "sigma_v", "sigma_w")
