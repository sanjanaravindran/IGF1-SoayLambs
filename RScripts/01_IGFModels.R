...
# Set wd
setwd("~/Desktop/IGF1_MS/FuncEcolSubmission/Data_Code")
source("00_functions.R")

# Read Data
igf_lh_data <- read.csv('IGF1_SoayLambs_Anon.csv',  header = T, stringsAsFactors = F, fileEncoding="UTF-8-BOM")
...
igf_hist <- ggplot(temp, aes(x=IGF1)) +
  geom_histogram() +
  xlab("Plasma IGF-1 Concentration (ng/ml)")
igf_hist
...
med_igf_df <- median_hdci(posterior_igf_df) #Median
p_value(mod_igf, method="hdi") #P-value
...
p1 <- bayesplot::mcmc_areas(predictors, point_est = c("median"), prob = 0.95, prob_outer = 0.95) +
  theme_cowplot() +
  vline_0(color = "darkgray", linetype = 2) +
  scale_x_continuous(limits=c(-110,30), n.breaks=10) +
...
plot1
# ggsave("./FigS1.tiff", plot1, dpi=600, width=6, height=4, bg="white" )
...
plot2
# ggsave("./Fig1.tiff", plot2, dpi=600, width=12, height=9, bg="white" )


color_scheme_set("viridisA")
p1_pp <- pp_check(mod_igf, nreps = 200) + labs(x="Plasma IGF-1 concentration (ng/ml)")
...
mod_igf_unscaled_st <- rstanarm::stan_lmer(IGF1 ~  SexF + Twin + PopSize + StorageTime + poly(MumAge, 2, raw = TRUE) + (1|ELISARunDate) + (1|PlateNumber) + (1|BirthYear) + (1|MumID),
                                           cores=4, 
                                           seed=12345,
                                           data=temp)
posterior_igf_df <- as.data.frame(mod_igf_unscaled_st)
med_igf_df <- t(median_hdci(posterior_igf_df))

mod_igf_st <- rstanarm::stan_lmer(IGF1 ~  SexF + Twin + PopSize_sc + StorageTime_sc + poly(MumAge_sc, 2, raw = TRUE) + (1|ELISARunDate) + (1|PlateNumber) + (1|BirthYear) + (1|MumID),
                                  cores=4, 
                                  seed=12345,
                                  data=temp)
...
# IGF-1 Model
# IGF-1 Model (continuous predictors are unscaled)
igf_lh_data$SampleCollectionTimeofDay <- igf_lh_data$CapHour_Tertiles
igf_lh_data$SampleCollectionTimeofDay <- factor(igf_lh_data$SampleCollectionTimeofDay, levels = c("Morning", "Afternoon", "Evening"))
levels(igf_lh_data$SampleCollectionTimeofDay)
temp_toc <- igf_lh_data %>%
...
mod_igf_toc <- rstanarm::stan_lmer(IGF1 ~  SexF + Twin + PopSize_sc + SampleCollectionTimeofDay + poly(MumAge_sc, 2, raw=TRUE) + (1|ELISARunDate) + (1|PlateNumber) + (1|BirthYear) + (1|MumID),
                                   cores=4, 
                                   seed=12345,
                                   data=temp_toc)
