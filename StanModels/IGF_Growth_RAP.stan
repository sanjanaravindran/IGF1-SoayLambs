data {
  ...
  vector[N] BirthWt_sc; //birthweight predictor
  vector[N] Age_sc; //age predictor
...
parameters {
  ...
  real beta_BirthWt;  // slope for birth wt in weight~IGF + ... model
  real beta_Age;  // slope for age in weight~IGF + ... model
  real beta_MumAge1; // slope for mum age(1) in weight~IGF + ... model
  real beta_MumAge2; // slope for mum age(2) in weight~IGF + ... model
  ...
  real<lower=0> sigma_e; //error sd for weight model
...
model {
 ...
 // likelihood
  Weight ~ normal(alpha + beta_IGF * IGF_true + beta_SexF * SexF + 
                    ...
                    beta_BirthWt * BirthWt_sc + beta_Age * Age_sc + 
...
generated quantities {
  ...
  for (i in 1:N)
    Weight_rep[i] = normal_rng(alpha  + beta_IGF * IGF_true[i] + beta_SexF * SexF[i] + 
                                 ...
                                 beta_BirthWt * BirthWt_sc[i] + beta_Age * Age_sc[i] +
